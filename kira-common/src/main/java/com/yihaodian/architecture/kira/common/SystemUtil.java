 ...
 * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4665037" >here</a> which oracle and sun
 * can't fix.
 *
 * @author Archer Jiang
 */
public class SystemUtil {
  ...
  public static String getJvmPid() {
    try {
      if (null == jvmPid || "".equals(jvmPid)) {
        jvmPid = ManagementFactory.getRuntimeMXBean().getName();
      }
    } catch (Exception e) {
      logger.error("Get jvm pid failed!!!", e);
    }
    return jvmPid;
  }
  ...
  public static int getPid() {
    if (null == pid) {
      String jvmPid = SystemUtil.getJvmPid();
      if (null != jvmPid && !"".equals(jvmPid.trim())) {
        try {
          pid = Integer.valueOf(jvmPid.substring(0, jvmPid.indexOf('@')));
        } catch (Exception e) {
  ...
  /**
   * Gets the address for the local machine. <p>
   *
   ...
   * IP address. <p> This method is intended for use as a replacement of JDK method
   ...
   * href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4665037" >here</a>. <p> This method
   * will scan all IP addresses on all network interfaces on the host machine to determine the IP
   * address most likely to be the machine's LAN address. If the machine has multiple IP addresses,
   * this method will prefer a site-local IP address (e.g. 192.168.x.x or 10.10.x.x, usually IPv4)
   * if the machine has one (and will return the first site-local address if the machine has more
   * than one), but if the machine does not hold a site-local address, this method will return
   ...
   * <p> <a href="http://issues.apache.org/jira/browse/JCS-40">JIR ISSUE JCS-40</a> <p>
   *
   * @return InetAddress
  ...
  private static InetAddress getLocalHostLANAddress() throws UnknownHostException {
    try {
      ...
      for (Enumeration<NetworkInterface> ifaces = NetworkInterface.getNetworkInterfaces();
        ...
        //if(iface.isUp()) { //Do not work in docker container.
