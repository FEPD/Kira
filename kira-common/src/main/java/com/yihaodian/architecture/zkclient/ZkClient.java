...
public class ZkClient implements Watcher {
   ...
   *
   * @throws ZkInterruptedException if operation was interrupted, or a required reconnection got
   * interrupted
   ...
   * ZkNodeExistsException} is thrown in case the path already exists
   * @throws ZkInterruptedException if operation was interrupted, or a required reconnection got
   * interrupted
   * @throws IllegalArgumentException if called from anything except the ZooKeeper event thread
   * @throws ZkException if any ZooKeeper exception occurred
  ...
  private void fireStateChangedEvent(final KeeperState state) {
    for (final IZkStateListener stateListener : _stateListener) {
      _eventThread.send(
          new ZkEventThread.ZkEvent("State changed to " + state + " sent to " + stateListener) {
  ...
  private void fireDataChangedEvents(final String path, Set<IZkDataListener> listeners) {
    for (final IZkDataListener listener : listeners) {
      _eventThread
          .send(new ZkEventThread.ZkEvent("Data of " + path + " changed sent to " + listener) {
            ...
            public void run() throws Exception {
              ...
              try {
                ...
                LOG.info("Can't deserialize object by java deserializer , use raw data directly!!!",
                    error);
                // 兼容其它客户端写入非对象序列化的数据
                byte[] data = null;
                try {
  ...
  private void fireChildChangedEvents(final String path, Set<IZkChildListener> childListeners) {
    try {
      ...
      for (final IZkChildListener listener : childListeners) {
        _eventThread.send(
            new ZkEventThread.ZkEvent("Children of " + path + " changed sent to " + listener) {
   ...
   * something without getting any zookeeper event interruption synchronize against this mutex. Also
   * all threads waiting on this mutex object will be notified on an event.
   *
   * @return the mutex.
   */
  ...
  /**
   * Updates data of an existing znode. The current content of the znode is passed to the {@link
   * DataUpdater} that is passed into this method, which returns the new content. The new content is
   * only written back to ZooKeeper if nobody has modified the given znode in between. If a
   * concurrent change has been detected the new data of the znode is passed to the updater once
   * again until the new contents can be successfully written back to ZooKeeper.
   *
  ...
  public void connect(final long maxMsToWaitUntilConnected, Watcher watcher)
      throws ZkInterruptedException, ZkTimeoutException,
    ...
    try {
      ...
      throw new IllegalStateException(
          "Not connected with zookeeper server yet. Current state is " + state);
    } finally {
      getEventLock().unlock();
